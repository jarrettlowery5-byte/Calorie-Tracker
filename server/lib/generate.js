...
export function buildImportPrompt({ text, url, servings }) {
  return [
    url
...
export async function importRecipe({ text, url, servings }) {
  if (!process.env.ANTHROPIC_API_KEY) {
    const err = new Error(
      "ANTHROPIC_API_KEY is not set. Copy .env.example to .env and add your key."
    );
  ...
  const response = await client.messages.parse({
    ...
    messages: [{ role: "user", content: buildImportPrompt({ text, url, servings }) }],
  ...
  if (!recipe) {
    ...
    if (!parsed.success) {
      const err = new Error("Couldn't read a recipe out of that.");
      err.status = 502;
      throw err;
    }
  ...
  if (recipe.name === "NOT_A_RECIPE" || !recipe.ingredients?.length) {
    const err = new Error("That didn't look like a recipe — try pasting the recipe text instead.");
    err.status = 422;
    throw err;
  }
  return recipe;
...
// Fetch a recipe page and reduce it to readable text. The backend can request
// the page directly; the browser build uses a reader service instead (CORS).
export async function fetchRecipePage(url) {
  ...
  if (!res.ok) {
    const err = new Error(`The site returned ${res.status} for that link.`);
    err.status = 422;
    throw err;
  }
