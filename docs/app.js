...
(() => {
  ...
  // window.api is provided by store.js (browser-only data layer)
  ...
  $('#setup-form').addEventListener('submit', async (e) => {
    ...
    if (f.get('api_key')) window.store.setApiKey(f.get('api_key'));
    delete body.api_key;
    try {
  ...
  function renderSettings() {
    ...
    form.append(
      ...
      el('div', { class: 'row2', style: 'margin-top:12px' },
    ...
    form.onsubmit = async (e) => {
      ...
      try {
        const { profile } = await api('/api/profile', { method: 'POST', body });
        state.profile = profile;
        window.store.setApiKey($('input[name="api_key"]', form).value);
        alert('Saved.');
      } catch (err) { alert(err.message); }
    };
  }
  ...
  function exportBackup() {
    ...
    const a = el('a', { href: URL.createObjectURL(blob), download: `calorie-tracker-backup-${localToday()}.json` });
    a.click();
    URL.revokeObjectURL(a.href);
  }

  const importInput = el('input', { type: 'file', id: 'import-file', accept: 'application/json,.json', hidden: '' });
  document.body.append(importInput);
  importInput.addEventListener('change', async (e) => {
    const file = e.target.files[0];
    if (!file) return;
    if (!confirm('Importing a backup replaces everything currently stored here. Continue?')) { e.target.value = ''; return; }
    try {
      window.store.importData(await file.text());
      location.reload();
    } catch (err) { alert(err.message); }
    e.target.value = '';
  });
